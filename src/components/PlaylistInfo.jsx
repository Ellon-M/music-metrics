...
function PlaylistInfo() {
  ...
  const details = useSelector((state) => state.details?.details[0]);
        ...
        {tracks && tracks.map((track) => (
          <li key={track.id} className="track">
            <div className="track-image-container">
              <img className="track-image" src={track.image} alt="track-album-art" />
            </div>
            <div className="track-info">
              <h5 className="track-name">{track.name}</h5>
              <span className="track-artist">
                {track.artists.map((artist) => <span className="track-artist-text" key={artist.id}>{artist}</span>)}
              </span>
            </div>
